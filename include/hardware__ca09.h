...
// The servers might have multiple CPUs. We limit all benchmarks to a single node using numactl.
// The listed CPU numbers below are for a single CPU. The listed NUMA numbers are just meant to
// give you a rough idea of the system.
...
// ARM: possible options are SVE, SVE2, and NEON. No ARM CPU older than Ampere Altra Max will be
// used.
#define SPC__SUPPORTS_NEON

// Cache information from `getconf -a | grep CACHE`.
// As Ubuntu did not list all numbers, we also took cache sizes from `cat
